...
class TestFindSubsets:
    ...
    def test_tolerance_is_in_integer_minor_units(self) -> None:
        ...
        assert find_subsets(amounts, 100_000, 5, max_group_size=3).solutions == [("A", "C", "B")]
...
class TestGroupMatcherIntegration:
    ...
    def test_one_to_many_settlement_is_grouped_and_suggested(self) -> None:
        ...
        assert sum(m.allocated_amount for m in group.members if m.side.value == "B") == Decimal(
            "10000.00"
        )
    ...
    def test_ambiguous_grouping_is_left_for_a_human(self) -> None:
        ...
        ledger = [
            make_transaction(
                "GL-1", "500.00", source_system="ledger", transaction_date=date(2026, 8, 31)
            ),
            make_transaction(
                "GL-2", "500.00", source_system="ledger", transaction_date=date(2026, 8, 31)
            ),
            make_transaction(
                "GL-3", "250.00", source_system="ledger", transaction_date=date(2026, 8, 31)
            ),
            make_transaction(
                "GL-4", "750.00", source_system="ledger", transaction_date=date(2026, 8, 31)
            ),
        ]
    ...
    def test_grouping_disabled_leaves_everything_alone(self) -> None:
        ...
        ledger = [
            make_transaction(
                "GL-1", "4000.00", source_system="ledger", transaction_date=date(2026, 8, 31)
            ),
            make_transaction(
                "GL-2", "6000.00", source_system="ledger", transaction_date=date(2026, 8, 31)
            ),
        ]
